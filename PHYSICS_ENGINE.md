...
| `mechanics/rolling.ts`             | Rolling without slipping: `SHAPE_FACTOR` (k = I/mr²), `momentOfInertia`, `rollingAcceleration` g sin θ/(1+k), `rollingTime`, `rollingSpeedAfterDrop`, energies, `rotationalShare`, `frictionNeededToRoll`                                                       | I per shape, a and ordering at many angles, mass and radius independence, energy conservation, v = ωr, μ_min < 0.80 over the range, invalid input |
...
### Rolling Race model (`src/simulations/physics/rolling-race`)

Full spec: `docs/labs/ROLLING_RACE_SPEC.md`.

| Aspect      | Choice                                                                                                                  |
| ----------- | ----------------------------------------------------------------------------------------------------------------------- |
| Bodies      | Three lanes. Each has a shape (solid sphere, solid cylinder, hollow sphere, ring), a mass and a radius                  |
| Motion      | Uniform acceleration a = g sin θ/(1 + k) from rest, evaluated in closed form; no integration                            |
| Rolling     | No slipping: v = ωr. Lanes have μs = 0.80, above the largest μ_min in range (0.50, a ring at 45°)                       |
| Termination | Each lane's clock stops when its centre crosses the finish; readings freeze there. The race ends when all have finished |
| Limits      | θ 2–45°, L 0.5–2.5 m, g 1.6–25 m/s², mass 0.1–5 kg, radius 20–120 mm                                                    |
